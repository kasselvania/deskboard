...
Run it from a clean checkout of `feat/private-homelab-manual-board` on the Mac that owns the existing signed Bridge state. Do not manually recreate its steps.
...
The API has no host-published port. The proxy is host-bound only to numeric loopback. Both pinned production images run non-root with read-only root filesystems. Tailscale Serve is the sole private HTTPS ingress; Funnel and public ingress are absent.
...
For prerequisites, the one-command owner procedure, normal consent, explicit **Sync Now**, device acceptance, and destination-only rollback, see [the Phase 3D deployment guide](../docs/private-homelab-deployment.md).
